...
def synthetic_dataframe(tmp_image_dir):
    ...
    return pd.DataFrame({
        ...
        "error_type":     ["TP" if i%4==0 else "TN" if i%4==1 else
                           "FP" if i%4==2 else "FN" for i in range(n)],
        "triage_tier":    ["Tier1" if i%4==0 else "Tier2" if i%4==1 else
                           "Normal" if i%4==2 else "Tier3" for i in range(n)],
    })
