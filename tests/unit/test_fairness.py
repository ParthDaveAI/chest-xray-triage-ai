...
def test_u26_gap_ci_point_estimate(fairness_df):
    ...
    """
    from src.fairness import compute_subgroup_recall, compute_gap_bootstrap_ci

    recalls = compute_subgroup_recall(fairness_df, "Patient Gender", n_bootstrap=50)

    pt, lo, hi = compute_gap_bootstrap_ci(
        fairness_df, "Patient Gender", "M", "F", n_bootstrap=50
    )
...
def test_u27_fairness_threshold_locked():
    ...
    assert FAIRNESS_THRESHOLD == approx(0.05), \
        "FAIRNESS_THRESHOLD must be 0.05 per Decision 16"
